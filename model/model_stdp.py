...
class STDPModel:
    ...
    def save_network_structure(self, chunk_size:int=50):
        """
    ...
    def add_item_loading_signals_mip(self, pop_id:list=[0], origin:list=[0.0], t_stop:list=[0.0]):
        """
        Add item loading signal to the pop_id-th excitatory population using the parameters previously given.
    ...
    def create_item_loading_signals_mip(self):
        ...
        t_stop = self.network_params["item_loading_mip"]["stop"]
    ...
    def simulate_network(self):
        ...
        stop_t_cue = self.simulation_params["t_stop_stdp"]
        step_ms = 100.0
        max_hz = 50.0 
        ...
        for step in range(int(total_time/step_ms)):
            ...
            if stop_t_cue != -1 and step*step_ms >= stop_t_cue:
                self.stop_stdp()
                stop_t_cue = -1
